 ...
 * interaction ultimately manifests to the page — checkpoint 1B.8 repair
 * round. `Locator.fill()` is built for typing into text-like inputs; a
 * `type="color"` control has no such affordance (the actual OS color
 * dialog it opens on click cannot be automated headlessly across
 * browsers/platforms), so this sets the element's `value` directly via its
 * real property setter and fires both `input` and `change` events — the
 * same two events a genuine pick fires, and exactly what React's
 * controlled-input value tracking (which is how `ColorField`'s `onChange`
 * ever gets called at all) listens for. This changes HOW the test drives a
 * native browser widget, not what the test asserts about the resulting
 * state — `ColorField`'s own component code and every synced-state
...
async function setNativeColorInputValue(page: Page, ariaLabel: string, hexValue: string): Promise<void> {
  await page.getByLabel(ariaLabel).evaluate((element, value) => {
