 ...
 * is the one place that calls `getCurrentTenantIdentity()` (checkpoint
 * 1B.6 — identity only, no map resolution; see `lib/tenant/tenant-identity.ts`)
 * for this purpose, reusing the exact same trusted tenant-resolution path
 * every other page already uses (no duplicated auth/authorization logic
 * here).
 *
  ...
  return (
    <header className="admin-header">
      <div className="admin-header-left">
        ...
        {/* checkpoint 1B.6 §7 — renders only inside a map's own routes; see MapSwitcher's own doc comment. */}
        <MapSwitcher />
      </div>
