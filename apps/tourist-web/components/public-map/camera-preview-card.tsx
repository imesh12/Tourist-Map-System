  ...
  return (
    <div className="camera-preview-card" data-testid="camera-preview-card">
      ...
      <div data-testid="camera-preview-footer" className="camera-preview-meta" role="button" tabIndex={0} onClick={onOpen} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') onOpen(); }}>
        <span className="camera-preview-live"><span className="camera-preview-live-dot" aria-hidden="true" /> {messages.live}</span>
        <strong data-testid="camera-preview-name">{camera.name}</strong>
        <span className="camera-preview-affordance">{messages.openDetails}</span>
      </div>
      <button type="button" className="camera-preview-close" aria-label={messages.closeCameraPreview} onClick={(event) => { event.stopPropagation(); onClose(); }}>×</button>
    </div>
  );
}
